...
function getStatusStyle(status: string) {
  return STATUS_STYLE[status] ?? "bg-[#E2E8F0] text-slate-700";
}
...
function OrderDetailModal({ order, onClose, onRefresh }: OrderDetailModalProps) {
  ...
  const handleChatWithBuyer = async () => {
    if (!order.emailKhachHang) return;
    try {
      ...
      console.error("Lỗi không tạo được conversation");
    }
  };
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="space-y-3">
              {order.chiTiet?.map((item) => (
                <Link key={item.maChiTietDonHang} to={`/product/${item.maSanPham}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">{(item.giaBan || 0).toLocaleString("vi-VN")}đ × {item.soLuong || 0}</p>
                  </div>
                  <p className="font-bold text-slate-800 flex-shrink-0">{(item.thanhTien || 0).toLocaleString("vi-VN")}đ</p>
                </Link>
              ))}
            </div>
          </div>
          ...
          {showCancelForm && (
            <div className="rounded-2xl border border-[#FDE8E8] bg-white p-4 space-y-3">
              ...
              <textarea value={lyDoHuy} onChange={(e) => setLyDoHuy(e.target.value)}
                placeholder="Nhập lý do hủy..." rows={3}
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <button onClick={handleChatWithBuyer}
...
function UserSellOrder() {
  ...
  const tabs: TabItem[] = useMemo(() => {
    ...
    const statusTabs: TabItem[] = (dbStatuses || []).map((s) => ({
      ...s, label: s.tenTrangThai, value: s.tenTrangThai,
    }));
  ...
  const handleTabClick = (tab: TabItem) => { setActiveTabValue(tab.value); setCurrentPage(1); };
    ...
    <div className="py-16 px-12">
      ...
      <div className="mt-8 rounded-3xl bg-[#F4FBEE] p-4 shadow-sm">
        <div className="flex flex-wrap gap-3">
          {tabs.map((tab) => (
            <button key={tab.value} onClick={() => handleTabClick(tab)}
              className={`rounded-full px-5 py-3 text-sm font-semibold transition ${activeTabValue === tab.value ? "bg-[#4E6A4E] text-white" : "bg-white text-slate-700 shadow-sm hover:bg-[#eaf5e0]"}`}>
              {tab.label}
            </button>
          ))}
        </div>
      ...
      <div className="mt-6 space-y-4">
        {loading && (
          <div className="rounded-3xl bg-[#F7FCF1] p-8 text-center text-slate-400">Đang tải...</div>
        )}

        {!loading && pageOrders.length === 0 && (
          <div className="rounded-3xl bg-[#F7FCF1] p-8 text-center text-slate-500">Không có đơn hàng phù hợp.</div>
        )}

        {!loading && pageOrders.map((order) => (
          <div key={order.maDonHang} onClick={() => setSelectedOrder(order)}
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-start gap-4">
                <img
                  src={order.chiTiet[0]?.hinhAnh || "https://via.placeholder.com/80x80?text=No+Image"}
                ...
                <div>
                  ...
                  <div className="mt-2 font-bold text-slate-800">{order.chiTiet[0]?.tenSanPham || "N/A"}</div>
              ...
              <div className="flex flex-col items-start gap-2 sm:items-end">
                <StatusBadge status={order.trangThai} />
                <div className="text-lg font-bold text-slate-800">{order.tongTien.toLocaleString("vi-VN")}đ</div>
          ...
          <div className="flex items-center gap-2">
            {Array.from({ length: pageCount }, (_, index) => (
              <button key={index} onClick={() => setCurrentPage(index + 1)}
                className={`h-10 w-10 rounded-full border text-sm font-semibold transition ${currentPage === index + 1 ? "border-[#4E6A4E] bg-[#4E6A4E] text-white" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-100"}`}>
                {index + 1}
              </button>
            ))}
          </div>
