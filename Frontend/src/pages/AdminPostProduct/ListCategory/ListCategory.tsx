...
type Props = { categoryId: number; handleChange: <K extends keyof UploadProduct>(field: K, value: UploadProduct[K]) => void; };
...
function ListCategory({ categoryId, handleChange }: Props) {
  ...
  useEffect(() => {
    ...
    publicAxios.get("/home/categories")
      .then((res) => setCategories((res.data as any[]).map((item) => new Category(item.maTheLoai, item.tenTheLoai))))
  ...
  return (
    <div className="space-y-2">
      ...
      <select className="w-full border border-gray-300 rounded-md p-3 focus:outline-none focus:border-[#4d5e45] bg-white appearance-none" value={categoryId} onChange={(e) => handleChange("categoryId", Number(e.target.value))}>
