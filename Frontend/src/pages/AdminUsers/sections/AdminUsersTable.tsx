...
const AdminUsersTable = ({ users, loading, onEdit, onDelete, onRestore }: Props) => (
    ...
    <table className="w-full text-left border-collapse">
      <thead>
        <tr className="border-b border-gray-100 bg-gray-50">
          ...
          <th className="py-5 px-6 text-sm font-medium text-gray-500 text-center">Quyền</th>
      ...
      <tbody>
        {loading ? (
          <tr><td colSpan={6} className="py-8 px-6 text-center text-gray-500">Đang tải dữ liệu...</td></tr>
        ) : users.length > 0 ? users.map((user) => (
          <tr key={user.maNguoiDung}
            className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${user.trangThai === 0 ? "opacity-60 bg-gray-100" : ""}`}>
            ...
            <td className="py-6 px-6 text-sm text-center">
              {user.roles && user.roles.length > 0 ? (() => {
                ...
                return (
                  <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-bold shadow-sm ${isAdmin ? "bg-red-100 text-red-700 border border-red-200" : "bg-blue-100 text-blue-700 border border-blue-200"}`}>
                    {cleanRole}
                  </span>
                );
            ...
            <td className="py-6 px-6 text-sm text-center">
              <div className="flex justify-center gap-3">
                <button onClick={() => onEdit(user)} className="p-2 hover:bg-yellow-100 text-yellow-600 rounded-lg transition-colors" title="Chỉnh sửa">
                  <Pencil size={18} />
                </button>
                {user.trangThai === 0 ? (
                  <button onClick={() => onRestore(user)} className="p-2 hover:bg-green-100 text-green-600 rounded-lg transition-colors" title="Khôi phục">
                    <RotateCcw size={18} />
                  </button>
                ) : (
                  <button onClick={() => onDelete(user)} className="p-2 hover:bg-red-100 text-red-600 rounded-lg transition-colors" title="Xóa">
                    <Trash2 size={18} />
                  </button>
                )}
              </div>
          ...
          <tr><td colSpan={6} className="py-8 px-6 text-center text-gray-500">Không tìm thấy người dùng nào</td></tr>
        )}
      </tbody>
    </table>
  </div>
