...
const tabs = [
  "Tất cả", "Chờ duyệt", "Đã duyệt", "Thành công", "Đã thanh toán", "Đã hủy",
] as const;
...
function UserBuyOrder() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const list: DonHangDTO[] = (res as any).data ?? res;
      setOrders(Array.isArray(list) ? list : []);

      // Kiểm tra đã đánh giá — đơn "Đã duyệt" hoặc "Thành công"
      const duyetOrders = list.filter(
        (o: DonHangDTO) => o.trangThai === "Đã duyệt" || o.trangThai === "Thành công",
      );
      const newMap: Record<number, boolean> = {};
      await Promise.all(
        duyetOrders.flatMap((order: DonHangDTO) =>
          (order.chiTiet ?? []).map(async (ct) => {
            try {
              const r = await kiemTraDaDanhGia(ct.maSanPham);
              newMap[ct.maSanPham] =
                (r as unknown as { data: { daDanhGia: boolean } }).data?.daDanhGia ?? false;
            } catch {
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      setOrders((prev) => prev.map((o) => (o.maDonHang === cancelOrderId ? updated : o)));
      closeCancelModal();
    } catch { setCancelError("Hủy đơn thất bại. Vui lòng thử lại."); }
    finally { setCancelling(false); }
  };

  const handleChatSeller = async (emailOpponent: string) => {
    if (!emailOpponent) return;
    try {
      const res = (await chatWithSeller(emailOpponent)) as any;
      const convId = res?.data?.conversationId ?? res?.data?.id;
      if (convId) {
        dispatch(chatSlice.actions.setConversationId(convId));
        navigate("/profile/messages");
      }
    } catch { console.error("Không thể tạo cuộc trò chuyện"); }
  };
  ...
  return (
    <div className="py-12 px-6 md:px-12 max-w-4xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2">
          {tabs.map((tab) => (
            <button key={tab} onClick={() => handleTabClick(tab)}
              className={`rounded-full px-4 py-2 text-sm font-semibold transition ${activeTab === tab ? "bg-[#4E6A4E] text-white" : "bg-white text-slate-700 shadow-sm hover:bg-slate-50"}`}>
              {tab}
              {tab !== "Tất cả" && (
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="rounded-2xl bg-[#F7FCF1] p-10 text-center text-slate-400">Đang tải đơn hàng...</div>
        ) : pageOrders.length === 0 ? (
          <div className="rounded-2xl bg-[#F7FCF1] p-10 text-center text-slate-500">Không có đơn hàng nào.</div>
          ...
          <div key={order.maDonHang} className="rounded-2xl bg-white border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-3 bg-[#F7FCF1] border-b border-slate-100">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {order.tenNguoiBan && (
                  <span className="flex items-center gap-1 text-xs text-[#4E6A4E] bg-[#E8F5E3] px-2 py-0.5 rounded-full">
                    <Store size={11} />
                    {order.tenNguoiBan}
                    {order.emailNguoiBan && <span className="text-[#4E6A4E]/70">· {order.emailNguoiBan}</span>}
                  </span>
                )}
                {order.phuongThucThanhToan && (
                  <span className="flex items-center gap-1 text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
                    <CreditCard size={11} /> {order.phuongThucThanhToan}
                  </span>
                )}
              </div>
              <div className="flex flex-row justify-center items-center gap-2">
                ...
                {order.emailNguoiBan && (
                  <button onClick={() => handleChatSeller(order.emailNguoiBan!)}
                    className="p-2 bg-gray-200 rounded-2xl hover:cursor-pointer hover:opacity-90 text-xs">
                    Nhắn tin với shop
                  </button>
                )}
              </div>
                ...
                <div key={ct.maChiTietDonHang} className="flex items-center gap-4">
                  <Link to={`/product/${ct.maSanPham}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-slate-800 truncate group-hover:text-[#4E6A4E] transition">{ct.tenSanPham}</p>
                      <p className="text-xs text-slate-500 mt-0.5">x{ct.soLuong} · {ct.giaBan.toLocaleString("vi-VN")}đ/cái</p>
                    </div>
                  ...
                  {(order.trangThai === "Đã duyệt" || order.trangThai === "Thành công") && (
                    daDanhGiaMap[ct.maSanPham] ? (
                      ...
                      <button onClick={() => setReviewItem(ct)}
        ...
        <div className="mt-8 flex items-center justify-center gap-2">
          {Array.from({ length: pageCount }, (_, i) => (
            <button key={i} onClick={() => setCurrentPage(i + 1)}
              className={`h-9 w-9 rounded-full border text-sm font-semibold transition ${currentPage === i + 1 ? "border-[#4E6A4E] bg-[#4E6A4E] text-white" : "border-slate-300 bg-white text-slate-700 hover:border-[#4E6A4E]"}`}>
              {i + 1}
            </button>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={closeCancelModal} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>
            <p className="text-sm text-slate-500 mb-4">Vui lòng chọn lý do hủy:</p>
            <div className="space-y-2 mb-4">
              {LY_DO_HUY.map((lyDo) => (
                <label key={lyDo}
                  className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition ${selectedReason === lyDo ? "border-[#4E6A4E] bg-[#F4FBEE]" : "border-slate-200 hover:border-slate-300"}`}>
                  <input type="radio" name="lyDoHuy" value={lyDo} checked={selectedReason === lyDo}
                    onChange={() => setSelectedReason(lyDo)} className="accent-[#4E6A4E]" />
