...
const MAX_IMAGES = 3;
export type ExistingImageEdit = { maHinhAnh: number; tenHinhAnh: string; duongDan: string; preview: string; markedDelete: boolean; replaceFile: File | null; replacePreview: string | null; };
...
function ImageEditor({ initialImages, setImages, setDeleteImage }: any) {
  ...
  useEffect(() => {
    setExisting(initialImages.map((img: any) => ({ maHinhAnh: img.maHinhAnh, tenHinhAnh: img.tenHinhAnh, duongDan: img.duongDan, preview: img.duongDan, markedDelete: false, replaceFile: null, replacePreview: null })));
  }, [initialImages]);
  ...
  const handleAddFiles = (files: FileList | null) => {
    if (!files) return;
    const slotsLeft = MAX_IMAGES - visibleCount; if (slotsLeft <= 0) return;
  ...
  const handleReplaceFile = (files: FileList | null) => {
    const id = replacingIdRef.current; if (!files || id == null) return;
    ...
    replacingIdRef.current = null; if (replaceInputRef.current) replaceInputRef.current.value = "";
  };

  useEffect(() => {
    const images = [
      ...existing.filter((img) => !img.markedDelete).map((img) => ({ maHinhAnh: img.maHinhAnh, tenHinhAnh: img.replaceFile ? img.replaceFile.name : img.tenHinhAnh, duongDan: img.replacePreview ?? img.duongDan, file: img.replaceFile })),
      ...added.map((img) => ({ maHinhAnh: null, tenHinhAnh: img.file.name, duongDan: img.preview, file: img.file })),
    ];
    setImages(images);
    setDeleteImage(existing.filter((img) => img.markedDelete).map((img) => img.maHinhAnh));
  }, [existing, added]);
      ...
      <div className="flex flex-wrap gap-3">
        {existing.map((img) => img.markedDelete ? (
          ...
          <div key={img.maHinhAnh} className="relative">
            ...
            <button type="button" onClick={() => setExisting((p) => p.map((i) => i.maHinhAnh === img.maHinhAnh ? { ...i, markedDelete: true, replaceFile: null, replacePreview: null } : i))} disabled={visibleCount <= 1} className="absolute -right-1 -top-1 rounded-full bg-white p-1 shadow disabled:opacity-40"><X className="h-4 w-4 text-gray-700" /></button>
        ...
        {added.map((img) => (
          <div key={img.tempId} className="relative">
            ...
            <button type="button" onClick={() => setAdded((p) => { const t = p.find((a) => a.tempId === img.tempId); if (t) URL.revokeObjectURL(t.preview); return p.filter((a) => a.tempId !== img.tempId); })} className="absolute -right-1 -top-1 rounded-full bg-white p-1 shadow"><X className="h-4 w-4 text-gray-700" /></button>
          </div>
        ))}
        {canAddMore && (
