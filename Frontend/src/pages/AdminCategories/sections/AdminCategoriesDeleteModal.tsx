    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
        <div className="p-6 border-b border-gray-200">
          <h3 className="text-xl font-bold text-brand-heading">Xác nhận xóa phân loại</h3>
        </div>

        <div className="p-6">
          <p className="text-sm text-gray-600 mb-2">Bạn có chắc chắn muốn xóa phân loại:</p>
          <p className="text-lg font-bold text-brand-heading mb-4">{category.tenTheLoai}</p>
          <p className="text-xs text-gray-500">Số lượng sản phẩm hiện tại: {category.soSanPham}</p>
          {hasProducts ? (
        ...
        <div className="flex gap-3 p-6 border-t border-gray-200 bg-gray-50">
          ...
          <button onClick={onConfirm} disabled={isDeleting || hasProducts}
            className={`flex-1 px-4 py-2 text-white font-semibold rounded-lg transition-colors ${hasProducts ? "bg-gray-400 cursor-not-allowed" : "bg-red-600 hover:bg-red-700 disabled:bg-gray-400"}`}>
